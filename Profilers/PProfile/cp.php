...
// mysqli_select_db("placement"); // Selecting Database from Server
	...
	if($Password && $repassword && $cur) 
	{
		...
		{
			$sql = mysqli_query($connect,"SELECT * FROM `plogin` WHERE `Username`='$Username'");
			if(mysqli_num_rows($sql) == 1)
			{
				...
				if($cur == $dbpassword)
				{
					if($query = mysqli_query($connect,"UPDATE `plogin` SET `Password` = '$Password' WHERE `plogin`.`Username` = '$Username'"))
					{
